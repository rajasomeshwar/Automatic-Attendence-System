...
// beliow abo studnet port mumber 8082
export const getAllStudentApiSpecific = async () => {
  //  const token = localStorage.getItem("jwtToken");
  //console.log("inside ther ", token);
  return await Student.get(`/students/4/pictures`);
};

export const getAllStudentApi = async () => {
  //  const token = localStorage.getItem("jwtToken");
  //console.log("inside ther ", token);
  return await Student.get(`/students/picturesAll`);
};

export const getAllStudentByDateOptionApi = async (d) => {
  //  const token = localStorage.getItem("jwtToken");
  //console.log("inside ther ", token);
  console.log(JSON.stringify(d) + " xx ");
  const date = d.d.date;
  const option = d.d.option;
  return await Student.get(`/students/s/${date}/${option}`);
};

export const getAllStudentByDateOptionDepthApi = async (d) => {
  //  const token = localStorage.getItem("jwtToken");
  //console.log("inside ther ", token);
  console.log(JSON.stringify(d) + " xx ");
  const date = d.d.date;
  const option = d.d.option;
  return await Student.get(`/students/d/${date}/${option}`);
};
